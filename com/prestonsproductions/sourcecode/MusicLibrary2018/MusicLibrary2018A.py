...
def getMusicLibraryTitle():
    ...
    return '1 - Bath High School Blue - Music Library'
...
def getAverageSize(library, attribute):
    if (attribute not in ['DURATION', 'FILESIZE', 'RATING']): return -1
    size = 0;
    for s in library:
        size += s[attribute]
    return size / len(library)
...
def getTheBestSong(library):
    ...
    ratio = 0
    for s in library:
        if (s['RATING'] == 5 and s['ARTIST'] != 'AWOLNATION'):
            tmp = s['DURATION'] / s['FILESIZE']
            if (tmp > ratio): 
                bestSongs = [s['NAME'] + ' ' + str(tmp)]
                ratio = tmp
            elif (tmp == ratio): bestSongs.append(s['NAME'] + ' ' + str(tmp))
    return bestSongs
...
def getShortestMixTape(library):
    cd = []
    library.sort(key=getDuration)
    count = 0
    prevDuration = 0
    for s in library:
        if (count < 10 or prevDuration == s['DURATION']):
            cd.append(s['NAME'])
...
def getMedianDuration(library):
    ...
    library.sort(key=getDuration)
    if (len(library) % 2 != 0):
        median = library[len(library)//2]['DURATION']
    else:
        median = (library[len(library)//2]['DURATION'] + library[(len(library)//2)-1]['DURATION']) / 2
    return median
...
def getAnagrams(library, phrase):
    ...
    for s in library:
        song = s['NAME'].replace(' ','')
        if (sorted(song.upper()) == sorted(phrase.upper())):
            anagrams.append(s['NAME'])
    return anagrams
...
def main():
    ...
    print('QUESTION 0 OBJECTIVE A (2): '+str(getMusicLibraryTitle())) # School Name
    print('QUESTION 1 OBJECTIVE A (2): '+str(getCDArea(5))) # 78.53981633974483
    print('QUESTION 2 OBJECTIVE A (2): '+str(hasEnoughSpace(1024,1024))) # True
    print('QUESTION 3 OBJECTIVE A (3): '+str(logSong('Love Me Do','The Beatles','1'))) # Love Me Do : The Beatles : 1
    print('QUESTION 4 OBJECTIVE A (3): '+str(formatTime(205))) # 3:25 or 3.4166666666666665
    print('QUESTION 4 OBJECTIVE B (3): '+str(formatTime(205))) # 3:25
    print('QUESTION 5 OBJECTIVE A (3): '+str(reverseString('Man in the Wilderness'))) # ssenredliW eht ni naM
    print('QUESTION 6 OBJECTIVE A (4): '+str(getSong(library, 'Soul Vaccination'))) # {'NAME': 'Soul Vaccination', 'DURATION': 297, 'ARTIST': 'Tower of Power', 'FILESIZE': 4.6, 'ALBUM': 'Soul Vaccination: Live', 'RATING': 5}
  
    print('QUESTION 7 OBJECTIVE A (4): '+str(getTotalTime(library))) # 226:14 or 226.23333333333332
    
    print('QUESTION 8 OBJECTIVE A (5): '+str(getAverageSize(library, 'RATING')) +' '+ str(getAverageSize(library, 'FILESIZE'))) # 3.685185185185185 3.511111111111111
    print('QUESTION 8 OBJECTIVE B (2): '+str(getAverageSize(library, 'RATINGSS'))) # -1
    ...
    print('QUESTION 13 OBJECTIVE A (5): '+str(capitalizeSongTitle('down to the night Club'))) # Down To The Night Club
    print('QUESTION 14 OBJECTIVE A (6): '+str(getAlbumMostSongs(library))) # Megalithic Symphony, Soul Vaccination: Live
    
    print('QUESTION 15 OBJECTIVE A (6): '+str(getShortestMixTape(library))) # [10, 'Cant Buy Me Love', 'Love Me Do', 'Eight Days A Week', 'Demons', 'My Fault', 'Wake Up', 'Knights of Shame', 'Numb', 'Beautiful Loser', 'Radioactive'] or ['Cant Buy Me Love', 'Love Me Do', 'Eight Days A Week', 'Demons', 'My Fault', 'Wake Up', 'Knights of Shame', 'Numb', 'Beautiful Loser', 'Radioactive']
    print('QUESTION 15 OBJECTIVE B (2): '+str(getShortestMixTape(library))) # [10, 'Cant Buy Me Love', 'Love Me Do', 'Eight Days A Week', 'Demons', 'My Fault', 'Wake Up', 'Knights of Shame', 'Numb', 'Beautiful Loser', 'Radioactive']
    
    print('QUESTION 16 OBJECTIVE A (7): '+str(getMedianDuration(library))) # 245.0
    print('QUESTION 17 OBJECTIVE A (7): '+str(fibonnaciSum(library))) # 377   
    print('QUESTION 18 OBJECTIVE A (8): '+str(getAnagrams(library, 'I hang under lo list'))) # ['The Grand Illusion']
